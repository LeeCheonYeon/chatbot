    ...
    response = ollama_client.chat(model=OLLAMA_MODEL, messages=[
            {
                ...
                "content": (
                    ...
                    "15. 자신[광주광역시도시공사 비즈니스 비서]을 소개하는 문장은 제외한다.\n"
                    "16. 날짜를 확인하여 단어를 선택하라.\n"
                    "17. 정확한 정보만 답변하세요.\n"
            )
            },
            {
...
def rewrite_query_with_history(user_id, current_query):
    ...
    history_text = "\n".join([f"{m['role']}: {m['content']}" for m in history])
    prompt = f"""
    [ 대화 이력] 과 [마지막 질문]을 바탕으로 사용자의 질문을 '단독으로 검색 가능한 문장'으로 다시 작성하세요.
    
    대화 이력: {history_text}
    마지막 질문: {current_query}
    
    [마지막 질문]을 분석하여 [대화 이력]과 관련된 질문을 정리해서 출력하세요.
    반드시 질문만 출력하세요. 
    절대 [대화 이력]에 있는 질문을 그대로 출력하지 마세요.
    절대 질문에 대한 답변을 하지마세요.
    다시 작성된 질문의 내용은 질문만 있고 어떻게 해서 질문이 나왔는지는 절대 포함되면 안됩니다.
    [대화 이력]과 관련없으면 [마지막 질문]을 출력하세요.
    한 문장만 출력하세요.
    """
