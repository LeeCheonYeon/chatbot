    ...
    response = ollama_client.chat(model=OLLAMA_MODEL, messages=[
        {
            ...
            "content": (
                ...
                "2. 맥락 유지: [이전대화]의 점수가 1.0에 가까울수록 가장 중요한 최신 맥락으로 취급하라.\n"
                "3. 독립성: [이전대화]와 관련 없는 새로운 질문이라면 [마지막 질문]을 수정 없이 그대로 출력하라.\n"
                "4. 독립성: [이전대화]와 관련 없는 주제라면 [마지막 질문]을 수정 없이 그대로 출력하라.\n"
                "5. 금지 사항: 절대로 질문에 답변하지 마라. 설명을 덧붙이지 마라. 상상해서 정보를 채우지 마라.\n"
                "6. 형식: 오직 '변환된 질문 한 문장'만 출력하라. 따옴표나 특수문자는 제거한다.\n\n"
