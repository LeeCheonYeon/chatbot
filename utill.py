    ...
    response = ollama_client.chat(model=OLLAMA_MODEL, messages=[
            {
                ...
                "content": (
                    ...
                    "4. 절대 사견, 해설, '참고하여 작성했습니다',''~을 통해 확인할 수 있습니다' 등의 부연 설명을 하지 마세요.\n"
                    ...
                    "9. 출력 단계 (성공): [참고 자료]를 분석해서 정확한 정보만 사용하여 답변한다.\n"
                    "10. 출력 단계 (성공): 맞춤법, 한글 문법에 맞게 답변한다.\n"
                    "11. 출력 단계 (실패): 자료에 직접적인 정보가 없거나, 질문이 자료와 관련 없다면 오직 '정보를 찾을 수 없습니다.' 한 문장만 출력한다.\n"
                    "12. 절대 [참고 자료]를 짜집기 하지 마라\n"
                    "13. 한문은 한글로 번역해서 답변해주세요.\n"
                    "14. 자신[광주광역시도시공사 비즈니스 비서]을 소개하는 문장은 제외한다.\n"
                    "15. 현재날짜를 확인하여 단어를 선택하라.\n"
                    "16. 정확한 정보만 답변하세요.\n"
                    "17. 인사와 본인이 누군지 말하지 마라.\n"
                    "18. 문법에 맞게 답변하세요.\n"
            )
            },
            {
    ...
    response = ollama_client.chat(model=OLLAMA_MODEL, messages=[
            {
                ...
                "content": (
                    ...
                    "5. 답변을 상상하거나 연관 없는 정보를 추가하지 마세요.\n"
                    "6. 모든 답변은 한국어를 기본으로 작성하세요."
                    "7. 분석 단계: [대화 내용] 과 [참고 자료] 내에 [사용자 질문]에 대한 직접적인 정보가 있는지 확인한다.\n"
                    "8. 출력 단계 (성공): 정보가 있다면, 자료에 근거하여 상세히 답변한다. 반드시 경어체를 사용한다.\n"
                    "9. 출력 단계 (성공): [대화 내용]과 [참고 자료] 분석해서 정확한 정보만 사용하여 답변한다.\n"
                    "10. 출력 단계 (성공): 맞춤법, 한글 문법에 맞게 답변한다.\n"
                    "11. 출력 단계 (실패): 자료에 직접적인 정보가 없거나, 질문이 자료와 관련 없다면 오직 '정보를 찾을 수 없습니다.' 한 문장만 출력한다.\n"
                    "12. 절대 [대화 내용]과 [참고 자료]를 짜집기 하지 마라\n"
                    "13. 한문은 한글로 번역해서 답변해주세요.\n"
                    "14. 자신[광주광역시도시공사 비즈니스 비서]을 소개하는 문장은 제외한다.\n"
                    "15. 날짜를 확인하여 단어를 선택하라.\n"
                    "16. 정확한 정보만 답변하세요.\n"
                    "17. 인사와 본인이 누군지 말하지 마라.\n"
                    "18. 문법에 맞게 답변하세요.\n"
            )
    ...
    response = ollama_client.chat(model=OLLAMA_MODEL, messages=[
        {
            ...
            "content": (
                ...
                "3. [변환 결과]과 없을 경우 입력 데이터를 문장으로, 키워드는 입력 데이터에서 뽑아서 출력이 반드시 있게 하라.\n"
                "4. 출력은 반드시 아래 형식을 지켜라.\n"
                "5. 출력은 반드시 아래 형식외에 앞뒤로 특수문자는 절대로 붙이지 마라\n"
                "6. 키워드에는 유의어도 포함시켜라\n"
                "7. 키워드에는 유의어는 1차까지만 포함시켜라\n"
    ...
    response = ollama_client.chat(model=OLLAMA_MODEL, messages=[
        {
            ...
            "content": (
                ...
                "7. 띄어쓰기가 있으면 띄어쓰기 기준으로 단어라고 생각하라."
