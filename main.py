...
def run_consulting_system():
    ...
    while True:
        ...
        user_query = util.remove_tag_text(user_query)
        
        #ollama를 통해서 질문을 검색용 문장 및 키워드로 변경
        vector_query, keyword_list = util.rewrite_question(user_query)
        print("="*60)
        print(vector_query)
        print(keyword_list)
        try:
            ...
            user_query_emb = util.get_embedding(vector_query)
            #initial_docs = search_collection_data("test_cylee",user_query_emb, count=20)
            initial_docs = util.search_collection_data_hybrid("test_cylee","full_contents",user_query_emb,keyword_list, 5)
            if not initial_docs:
            ...
            answer = util.ask_ollama(user_query, context_text)
